...
const defaultDialCooldown = 3 * time.Minute
...
type AuthRPCClient struct {
	...
	lastAuthRetryTime time.Time   // Last time we did try to authenticate this client
	conn              net.Conn    // Connection to control request timeouts
}
...
// clears lasRetryTime,this basically means that we will try again on next call
// When starting software, this is quite useful
func (authClient *AuthRPCClient) FlushLastRetryTime() {
	authClient.lastAuthRetryTime = time.Time{}
}
...
func (authClient *AuthRPCClient) Login() (err error) {
	...
	if authClient.authToken == "" && authClient.lastAuthRetryTime.Add(defaultDialCooldown).Before(time.Now()) {
		...
		if err != nil {
			...
			case *net.OpError:
				...
				if err.(*net.OpError).Timeout() {
					authClient.lastAuthRetryTime = time.Now()
				}
			}
...
func (authClient *AuthRPCClient) call(serviceMethod string, args interface {
	...
	// Flush timeout, so our readloop does not fail
...
// If there is a valid rpc.Client, it returns that else creates a new one.
